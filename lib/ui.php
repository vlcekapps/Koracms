...
/**
 * Vrátí odkaz na sdílené styly pro samostatné administrační přihlašovací obrazovky.
 */
function adminLoginStylesheetTag(): string
{
    return '  <link rel="stylesheet" href="' . h(BASE_URL . '/admin/assets/login.css') . '">' . "\n";
}

/**
 * Vrátí odkaz na sdílené styly pro administrační content/media picker.
 */
function adminContentReferencePickerStylesheetTag(): string
{
    return '  <link rel="stylesheet" href="' . h(BASE_URL . '/admin/assets/content-reference-picker.css') . '">' . "\n";
}
...
function sortableJs(): string
{
    ...
    return '<script nonce="' . $nonce . '">'
        ...
        . 'var feedbackAnchor=list.closest("table")||list;'
        ...
        . '.then(function(d){'
        . 'var msg=d&&d.ok?"Pořadí uloženo.":"Uložení pořadí selhalo. Zkontrolujte připojení a zkuste změnu znovu.";'
        ...
        . 'var dn=document.createElement("button");dn.type="button";'
