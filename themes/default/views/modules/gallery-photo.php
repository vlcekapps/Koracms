<div class="listing-shell">
  <section class="surface surface--accent" aria-labelledby="gallery-photo-title">
    <nav aria-label="Drobečková navigace">
      <ol class="breadcrumb-list">
        ...
        <?php foreach ($trail as $crumb): ?>
          <li><a href="<?= BASE_URL ?>/gallery/album.php?id=<?= (int)$crumb['id'] ?>"><?= h($crumb['name']) ?></a></li>
        <?php endforeach; ?>
    ...
    <figure class="photo-figure">
      <img class="photo-figure__image" src="<?= BASE_URL ?>/uploads/gallery/<?= rawurlencode($photo['filename']) ?>" alt="<?= h($photoTitle) ?>">
      <?php if ($photo['title'] !== ''): ?>
    ...
    <nav class="button-row button-row--start" aria-label="Navigace mezi fotografiemi">
      <?php if ($prevId): ?>
        <a class="button-secondary" href="<?= BASE_URL ?>/gallery/photo.php?id=<?= (int)$prevId ?>">← Předchozí</a>
      <?php else: ?>
        <span class="button-secondary button-secondary--disabled" aria-hidden="true">← Předchozí</span>
      <?php endif; ?>

      <?php if ($nextId): ?>
        <a class="button-secondary" href="<?= BASE_URL ?>/gallery/photo.php?id=<?= (int)$nextId ?>">Následující →</a>
      <?php else: ?>
        <span class="button-secondary button-secondary--disabled" aria-hidden="true">Následující →</span>
      <?php endif; ?>

      <a class="button-secondary" href="<?= BASE_URL ?>/gallery/album.php?id=<?= (int)$albumId ?>">Zpět do alba</a>
    </nav>
  </section>
</div>
