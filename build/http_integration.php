...
try {
    ...
    httpIntegrationClearLocalRateLimits($pdo, ['login']);
    $defaultRateLimitResponse = ['status' => '', 'headers' => [], 'body' => ''];
    for ($rateLimitAttempt = 0; $rateLimitAttempt < 6; $rateLimitAttempt++) {
        $defaultRateLimitResponse = postRawUrl(
            $baseUrl . BASE_URL . '/admin/login.php',
            'email=rate-limit@example.test&heslo=invalid',
    ...
    if (
        !httpIntegrationHeaderContains($defaultRateLimitResponse, 'Retry-After', '300')
    ...
    $pdo->prepare(
        "INSERT INTO cms_settings (`key`, value) VALUES ('module_reservations', '1')
    ...
    if (httpIntegrationStatusCode($foodItemsAdminPage) !== 200
        ...
        || !str_contains($foodItemsAdminPage['body'], 'Přidat položku')
        || !str_contains($foodItemsAdminPage['body'], 'Nejprve přidejte alespoň jednu sekci lístku.')) {
    ...
    if ($foodSectionId > 0) {
        $itemResponse = postUrl($foodItemsAdminUrl, [
            'csrf_token' => $adminSession['csrf'],
            'action' => 'save_item',
            'card_id' => (string)$foodCardId,
            'item_id' => '0',
            'section_id' => (string)$foodSectionId,
            'title' => 'HTTP Smažený sýr',
            'description' => 'S bramborem a domácí tatarkou.',
            'price_amount' => '129,90',
            'price_currency' => 'CZK',
            'price_note' => 'za porci',
        ...
        if ($foodItemId <= 0) {
            $foodStructuredIssues[] = 'položka strukturovaného lístku se neuložila do databáze';
        } else {
    ...
    $foodCardPath = foodCardPublicPath(['slug' => $foodSlug]);
    ...
    if (httpIntegrationStatusCode($foodArchiveResponse) !== 200
        || !str_contains($foodArchiveResponse['body'], $foodTitle)
        || !str_contains($foodArchiveResponse['body'], 'HTTP Smažený sýr')) {
    ...
    $publicFormFields = [
        ...
        [
            ...
            'default_value' => '',
            'help_text' => 'Volitelné. Uveďte ulici a číslo domu.',
            'options' => '',
            'accept_types' => '',
            'max_file_size_mb' => 1,
            'allow_multiple' => 0,
        ...
        [
            ...
            'placeholder' => '110 00',
            'default_value' => '',
            'help_text' => 'Volitelné. Poštovní směrovací číslo.',
            'options' => '',
            'accept_types' => '',
            'max_file_size_mb' => 1,
            'allow_multiple' => 0,
        ...
        [
            ...
            'placeholder' => 'Praha',
            'default_value' => '',
            'help_text' => 'Volitelné. Město nebo obec.',
            'options' => '',
            'accept_types' => '',
            'max_file_size_mb' => 1,
            'allow_multiple' => 0,
        ...
        [
            ...
            'placeholder' => 'Praha',
            'default_value' => '',
            'help_text' => 'Volitelné. Kraj nebo region.',
            'options' => '',
            'accept_types' => '',
            'max_file_size_mb' => 1,
            'allow_multiple' => 0,
        ...
        [
            ...
            'placeholder' => 'Česko',
            'default_value' => '',
            'help_text' => 'Volitelné. Země doručení nebo pobytu.',
            'options' => '',
            'accept_types' => '',
            'max_file_size_mb' => 1,
            'allow_multiple' => 0,
        ...
        [
            ...
            'name' => 'job_title',
            'placeholder' => 'Koordinátor',
            'default_value' => '',
            'help_text' => 'Volitelné. Funkce nebo pracovní pozice.',
            'options' => '',
            'accept_types' => '',
            'max_file_size_mb' => 1,
            'allow_multiple' => 0,
        ...
        [
            ...
            'placeholder' => '',
            'default_value' => '',
            'help_text' => 'Volitelné. Použijte datum narození, pokud ho formulář opravdu potřebuje.',
            'options' => '',
            'accept_types' => '',
            'max_file_size_mb' => 1,
            'allow_multiple' => 0,
    ...
    if (!httpIntegrationInputHasAttributes($publicFormInitialBody, 'field-full_name', ['autocomplete' => 'name'])
        ...
        || !httpIntegrationInputHasAttributes($publicFormInitialBody, 'field-street_address', ['autocomplete' => 'street-address'])
        || !httpIntegrationInputHasAttributes($publicFormInitialBody, 'field-postal_code', ['autocomplete' => 'postal-code'])
        || !httpIntegrationInputHasAttributes($publicFormInitialBody, 'field-city', ['autocomplete' => 'address-level2'])
        || !httpIntegrationInputHasAttributes($publicFormInitialBody, 'field-region', ['autocomplete' => 'address-level1'])
        || !httpIntegrationInputHasAttributes($publicFormInitialBody, 'field-country', ['autocomplete' => 'country-name'])
        || !httpIntegrationInputHasAttributes($publicFormInitialBody, 'field-job_title', ['autocomplete' => 'organization-title'])
        || !httpIntegrationInputHasAttributes($publicFormInitialBody, 'field-birth_date', ['autocomplete' => 'bday'])
