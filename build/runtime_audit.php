...
$httpServerRouterSelftestSource = is_file(__DIR__ . '/http_server_router_selftest.php') ? (string) file_get_contents(__DIR__ . '/http_server_router_selftest.php') : '';
...
$testRunLockSelftestSource = is_file(__DIR__ . '/test_run_lock_selftest.php') ? (string) file_get_contents(__DIR__ . '/test_run_lock_selftest.php') : '';
...
$foundationChecks = [
    ...
    'developer module guide exists' => str_contains($developerModulesDocSource, '# Vývoj nového modulu v Kora CMS')
        ...
        && str_contains($developerModulesDocSource, 'sitemap_sections')
        && str_contains($developerModulesDocSource, 'stats_page_types')
    ...
    'module contract audit is wired into module-ready CI' => str_contains($composerSource, '"test:module-contract"')
        ...
        && str_contains($moduleContractAuditSource, 'stats_page_types')
        ...
        && str_contains($moduleContractAuditSource, 'moduleContractAuditPublicEntryPointExists')
        ...
        && str_contains($moduleContractAuditSource, 'moduleContractAuditValidatePublicEntryPointStaticCoverage')
        ...
        && str_contains($moduleContractAuditSource, 'moduleContractAuditValidateAdminRouteModuleRequirements')
        && str_contains($moduleContractAuditSource, 'moduleContractAuditValidateAdminRouteStaticCoverage')
        && str_contains($moduleContractAuditSource, 'moduleContractAuditValidatePublicNavStaticCoverage')
        && str_contains($moduleContractAuditSource, 'must be covered by an analyse:strict composer script')
        && str_contains($moduleContractAuditSource, 'must be covered by a format:check composer script')
        && str_contains($moduleContractAuditSource, 'public_nav entrypoint')
        && str_contains($moduleContractAuditSource, 'moduleContractAuditExtractAdminRouteRequirementBlocks')
        && str_contains($moduleContractAuditSource, 'function adminRouteModuleRequirements(')
        && str_contains($moduleContractAuditSource, 'adminRouteModuleRequirement() must use the shared adminRouteModuleRequirements() map')
        ...
        && str_contains($moduleContractAuditSource, 'public_paths')
        && str_contains($moduleContractAuditSource, 'public module entrypoint')
        && str_contains($moduleContractAuditSource, 'must guard access with requireModuleEnabled')
        && str_contains($moduleContractAuditSource, 'adminRouteModuleRequirement entry')
        && str_contains($moduleContractAuditSource, 'must be listed in adminRouteModuleRequirement()')
        && str_contains($moduleContractAuditSource, 'adminRouteModuleRequirement file')
        && str_contains($moduleContractAuditSource, 'must call requireLogin(), requireSuperAdmin(), requireModuleEnabled() or requireCapability()')
        && str_contains($moduleContractAuditSource, 'admin_label')
        ...
        && str_contains($moduleContractAuditSource, 'moduleContractAuditValidateSitemapSectionCoverage')
        && str_contains($moduleContractAuditSource, 'content_reference_types')
        && str_contains($moduleContractAuditSource, 'search_result_types')
        && str_contains($moduleContractAuditSource, 'sitemap_sections')
        ...
        && str_contains($moduleContractAuditSource, 'moduleContractAuditValidateModuleSettingReferences')
        && str_contains($moduleContractAuditSource, 'moduleContractAuditValidateApplicationModuleSettingReferences')
        && str_contains($moduleContractAuditSource, 'moduleContractAuditRequireDocumentationFragments')
        && str_contains($moduleContractAuditSource, 'moduleContractAuditValidateDeveloperDocumentation')
        ...
        && str_contains($httpIntegrationFoundationSource, "httpIntegrationPrintResult('content_reference_disabled_modules_http'")
        && str_contains($httpIntegrationFoundationSource, 'picker nabízí vypnutý typ obsahu')
        && str_contains($httpIntegrationFoundationSource, 'content picker vrátil položku vypnutého modulu')
        && str_contains($moduleContractAuditSelftestSource, 'Unknown content picker module gate')
        && str_contains($moduleContractAuditSelftestSource, 'Missing admin route module requirement target')
        && str_contains($moduleContractAuditSelftestSource, 'Missing shared admin route requirement map')
        && str_contains($moduleContractAuditSelftestSource, 'Unknown admin route module requirement key')
        && str_contains($moduleContractAuditSelftestSource, 'Missing manifest admin path in route requirement map')
        && str_contains($moduleContractAuditSelftestSource, 'Missing admin route login guard')
        && str_contains($moduleContractAuditSelftestSource, 'Missing admin route PHPStan coverage')
        && str_contains($moduleContractAuditSelftestSource, 'Missing admin route format coverage')
        && str_contains($moduleContractAuditSelftestSource, 'Missing public navigation PHPStan coverage')
        && str_contains($moduleContractAuditSelftestSource, 'Missing public navigation format coverage')
        ...
        && str_contains($moduleContractAuditSelftestSource, 'Missing public paths manifest field')
        && str_contains($moduleContractAuditSelftestSource, 'Missing public navigation path in public paths')
        && str_contains($moduleContractAuditSelftestSource, 'Missing public module entrypoint target')
        && str_contains($moduleContractAuditSelftestSource, 'Missing public module entrypoint gate')
        && str_contains($moduleContractAuditSelftestSource, 'Missing public module entrypoint PHPStan coverage')
        && str_contains($moduleContractAuditSelftestSource, 'Missing public module entrypoint format coverage')
        ...
        && str_contains($moduleContractAuditSelftestSource, 'Missing developer module documentation fragment')
        && str_contains($moduleContractAuditSelftestSource, 'Missing README module documentation fragment')
        && str_contains($moduleContractAuditSelftestSource, 'Missing admin guide module documentation fragment')
    ...
    'core module metadata is centralized' => str_contains($definitionsSource, 'function coreModuleDefinitions()')
        ...
        && str_contains($definitionsSource, "'sitemap_sections'")
        && str_contains($definitionsSource, "'stats_page_types'")
    ...
    'admin module entrypoints enforce module state' => str_contains($authSource, 'function requireModuleEnabled(')
        ...
        && str_contains($unitTestsSource, 'foreach (adminRouteModuleRequirements() as $moduleKey => $requirement)')
        && str_contains($unitTestsSource, 'assert_admin_route_module_requirement_map_entry')
        ...
        && str_contains($unitTestsSource, "adminRouteModuleDisabledMessage('podcast')")
    ...
    'github actions workflow audit is wired into basic CI' => str_contains($composerSource, '"test:workflow"')
        ...
        && str_contains($composerSource, 'build/http_server_router_selftest.php')
        ...
        && str_contains($phpstanConfigSource, 'build/http_server_router_selftest.php')
    ...
    'http server router self-test is wired into basic CI' => str_contains($composerSource, '"test:http-router-selftest"')
        && str_contains($composerSource, 'php build/http_server_router_selftest.php')
        && str_contains($composerSource, '@test:http-router-selftest')
        && str_contains($composerSource, 'build/http_server_router_selftest.php')
        && str_contains($phpstanConfigSource, 'build/http_server_router.php')
        && str_contains($phpstanConfigSource, 'build/http_server_router_selftest.php')
        && str_contains($httpServerRouterSource, "routeToScript('index.php')")
    ...
    'external URL normalization uses shared unsafe-target guardrails' => str_contains($authSource, 'function normalizeHttpExternalUrl(string $target, bool $prependScheme = true): string')
        ...
        && str_contains($unitTestsSource, "normalizeContentEmbedUrl('//example.com/audio.mp3')")
    ...
    'repository guardrails audit is wired into local quality gates' => str_contains($composerSource, '"test:repo-guardrails"')
        ...
        && str_contains($composerSource, '"test:repo-guardrails-selftest"')
        && str_contains($composerSource, 'php build/repository_guardrails_audit_selftest.php')
        && str_contains($composerSource, '@test:repo-guardrails')
        && str_contains($composerSource, '@test:repo-guardrails-selftest')
        && str_contains($composerSource, 'build/repository_guardrails_audit_selftest.php')
        ...
        && str_contains($phpstanConfigSource, 'build/repository_guardrails_audit_selftest.php')
        && str_contains($repositoryGuardrailsAuditSource, 'repositoryGuardrailsProjectRoot')
        && str_contains($repositoryGuardrailsAuditSource, 'forbiddenTrackedRepositoryArtifactReason')
        && str_contains($repositoryGuardrailsAuditSource, 'sensitive local configuration must not be tracked')
        && str_contains($repositoryGuardrailsAuditSource, 'user upload content must not be tracked')
        && str_contains($repositoryGuardrailsAuditSource, 'Node dependencies must not be tracked')
        && str_contains($repositoryGuardrailsAuditSource, 'local Codex metadata must not be tracked')
        && str_contains($repositoryGuardrailsAuditSource, 'local Cursor metadata must not be tracked')
        && str_contains($repositoryGuardrailsAuditSource, 'uploads/.htaccess')
        && str_contains($repositoryGuardrailsAuditSource, '.env')
        ...
        && str_contains($repositoryGuardrailsAuditSelftestSource, 'assertRepositoryGuardrailsPasses')
        && str_contains($repositoryGuardrailsAuditSelftestSource, 'assertRepositoryGuardrailsFails')
        && str_contains($repositoryGuardrailsAuditSelftestSource, 'config.php')
        && str_contains($repositoryGuardrailsAuditSelftestSource, '.env')
        && str_contains($repositoryGuardrailsAuditSelftestSource, 'vendor/autoload.php')
        && str_contains($repositoryGuardrailsAuditSelftestSource, 'node_modules/example/index.js')
        && str_contains($repositoryGuardrailsAuditSelftestSource, '.codex/settings.json')
        && str_contains($repositoryGuardrailsAuditSelftestSource, '.cursor/rules/project.md')
        && str_contains($repositoryGuardrailsAuditSelftestSource, 'uploads/media/file.pdf')
        && str_contains($repositoryGuardrailsAuditSelftestSource, 'Reserved DB variable guard')
        && str_contains($repositoryGuardrailsAuditSelftestSource, "repositoryGuardrailsSelfTestVariable('user')"),
    ...
    'source encoding audit is wired into local quality gates' => str_contains($composerSource, '"test:encoding"')
        ...
        && str_contains($composerSource, '"test:encoding-selftest"')
        && str_contains($composerSource, 'php build/source_encoding_audit_selftest.php')
        && str_contains($composerSource, '@test:encoding-selftest')
        && str_contains($composerSource, 'build/source_encoding_audit.php')
        && str_contains($composerSource, 'build/source_encoding_audit_selftest.php')
        && str_contains($phpstanConfigSource, 'build/source_encoding_audit.php')
        && str_contains($phpstanConfigSource, 'build/source_encoding_audit_selftest.php')
        && str_contains($sourceEncodingAuditSource, 'sourceEncodingAuditProjectRoot')
        ...
        && str_contains($sourceEncodingAuditSelftestSource, 'assertSourceEncodingAuditPasses')
        && str_contains($sourceEncodingAuditSelftestSource, 'assertSourceEncodingAuditFails')
        && str_contains($sourceEncodingAuditSelftestSource, 'README.md: invalid UTF-8 encoding')
        && str_contains($sourceEncodingAuditSelftestSource, 'docs/admin-guide.md: unexpected UTF-8 BOM')
        && str_contains($sourceEncodingAuditSelftestSource, 'uploads/.htaccess: invalid UTF-8 encoding')
        && str_contains($sourceEncodingAuditSelftestSource, 'vendor/ignored.php'),
    ...
    'php lint self-test is wired into basic CI' => str_contains($composerSource, '"lint": "php build/lint_php.php"')
        && str_contains($composerSource, '"lint:selftest": "php build/lint_php_selftest.php"')
        && str_contains($composerSource, '@lint')
        && str_contains($composerSource, '@lint:selftest')
        && str_contains($composerSource, 'build/lint_php_selftest.php')
        && str_contains($phpstanConfigSource, 'build/lint_php.php')
        && str_contains($phpstanConfigSource, 'build/lint_php_selftest.php')
        && str_contains($lintPhpSource, 'lintPhpProjectRoot')
        && str_contains($lintPhpSource, 'KORA_LINT_PHP_ROOT')
        && str_contains($lintPhpSource, 'PHP lint failed for')
        && str_contains($lintPhpSelftestSource, 'assertLintPhpPasses')
        && str_contains($lintPhpSelftestSource, 'assertLintPhpFails')
        && str_contains($lintPhpSelftestSource, 'vendor/broken.php')
        && str_contains($lintPhpSelftestSource, 'dist/broken.php')
    ...
    'database-backed audits use shared run lock' => is_file(__DIR__ . '/test_run_lock.php')
        ...
        && str_contains($composerSource, 'build/test_run_lock_selftest.php')
    ...
    'theme view audit checks static id, label and aria references' => str_contains($themeViewAuditSource, 'themeViewAuditCheckStaticIdReferences')
        ...
        && str_contains($themeViewAuditSource, 'table without caption or aria-labelledby')
        && str_contains($themeViewAuditSource, 'table using aria-label instead of a caption or aria-labelledby')
        ...
        && str_contains($themeViewAuditSource, 'target="_blank" link without hidden DOM new-window text')
        && str_contains($themeViewAuditSource, 'target="_blank" link using aria-label instead of hidden DOM new-window text')
        && str_contains($themeViewAuditSource, 'themeViewAuditElementHasNewWindowText')
        && str_contains($themeViewAuditSource, 'newWindowLinkSrOnlySuffix()')
        && str_contains($themeViewAuditSelftestSource, 'Hidden new-window text helper guard')
        && str_contains($themeViewAuditSelftestSource, 'Blank target aria-label guard')
        ...
        && str_contains($themeViewAuditSelftestSource, 'Table accessible name guard')
        && str_contains($themeViewAuditSelftestSource, 'Table aria-label guard')
        ...
        && str_contains($themeViewAuditSelftestSource, 'Blank target hidden text guard')
        && str_contains($themeViewAuditSelftestSource, 'Blank target aria-label guard'),
    ...
    'release zip excludes dev tooling and metadata' => str_contains($releaseScriptSource, "'vendor'")
        ...
        && str_contains($releaseScriptSource, "'.codex'")
        && str_contains($releaseScriptSource, "'.cursor'")
    ...
    'release package audit is wired into basic CI' => str_contains($composerSource, '"test:release-package"')
        ...
        && str_contains($releasePackageAuditSource, "'.codex'")
        && str_contains($releasePackageAuditSource, "'.cursor'")
        && str_contains($releasePackageAuditSource, "'node_modules'")
    ...
    'release dry run smoke is wired into basic CI' => str_contains($composerSource, '"test:release-smoke"')
        ...
        && str_contains($releaseSmokeSource, 'Release smoke ZIP unexpectedly contains local Codex metadata')
        && str_contains($releaseSmokeSource, 'Release smoke ZIP unexpectedly contains Node dependencies')
        && str_contains($releaseSmokeSource, 'Source archive unexpectedly contains local Cursor metadata')
    ...
    'repository attributes protect source archives and line endings' => str_contains($gitattributesSource, '.gitattributes text eol=lf')
        ...
        && str_contains($gitattributesSource, '.github/** export-ignore')
        && str_contains($gitattributesSource, '.codex export-ignore')
        && str_contains($gitattributesSource, '.codex/** export-ignore')
        && str_contains($gitattributesSource, '.cursor export-ignore')
        && str_contains($gitattributesSource, '.cursor/** export-ignore')
        ...
        && str_contains($gitattributesSource, 'node_modules/** export-ignore')
    ...
    'gitignore protects local and generated artifacts' => str_contains($gitignoreSource, 'config.php')
        ...
        && str_contains($gitignoreSource, 'node_modules/')
        ...
        && str_contains($gitignoreSource, '.codex/')
        && str_contains($gitignoreSource, '.cursor/')
    ...
    'read-only discovery endpoints enforce HTTP methods' => str_contains($sitemapSource, '$isHeadRequest = requireReadOnlyHttpMethod();')
        ...
        && str_contains($authSource, 'function sendReadOnlyNotFoundResponse(string $message')
        && str_contains($authSource, 'sendNoStoreNoIndexHeaders();')
        && str_contains($authSource, "sendContentTypeNoSniffHeaders('text/plain; charset=UTF-8');")
        ...
        && str_contains($feedSource, "sendReadOnlyNotFoundResponse('Blog nebyl nalezen.', \$isHeadRequest);")
        && str_contains($feedSource, "if (\$isHeadRequest)")
        && !str_contains($feedSource, 'http_response_code(404)')
        ...
        && str_contains($podcastFeedSource, "sendReadOnlyNotFoundResponse('Podcast nebyl nalezen.', \$isHeadRequest);")
        && !str_contains($podcastFeedSource, 'http_response_code(404)')
        ...
        && str_contains($eventIcsSource, "sendReadOnlyNotFoundResponse('Událost nebyla nalezena.', \$isHeadRequest);")
        && !str_contains($eventIcsSource, 'http_response_code(404)')
    ...
    'sensitive token and logout endpoints send no-store noindex headers' => str_contains($authSource, 'function sendNoStoreNoIndexHeaders')
        && str_contains($authSource, 'function sendNoSniffHeader(): void')
        ...
        && str_contains($htaccessSource, '<IfModule mod_expires.c>')
        && str_contains($htaccessSource, 'ExpiresActive On')
        && str_contains($htaccessSource, 'ExpiresByType text/html M0')
    ...
    'file response helper enforces read-only methods' => str_contains($fileDownloadHelperSource, 'function requireReadOnlyHttpMethod(): bool')
        ...
        && str_contains($fileDownloadHelperSource, 'function storedFileContentDisposition(')
        && str_contains($fileDownloadHelperSource, "filename*=UTF-8\\'\\'")
        && str_contains($fileDownloadHelperSource, 'function storedFileEtag(')
        && str_contains($fileDownloadHelperSource, 'function storedFileRequestValidatorsMatch(')
        && str_contains($fileDownloadHelperSource, "header('ETag: ' . \$etag);")
        && str_contains($fileDownloadHelperSource, 'http_response_code(304);')
        && str_contains($fileDownloadHelperSource, 'function sendInlineStoredFileResponse(')
        && str_contains($fileDownloadHelperSource, "header('Content-Disposition: ' . storedFileContentDisposition('inline', \$downloadName));")
        && str_contains($fileDownloadHelperSource, "header('Content-Disposition: ' . storedFileContentDisposition(\$disposition, \$downloadName));")
        && str_contains($fileDownloadHelperSource, 'sendNoSniffHeader();')
        && !str_contains($fileDownloadHelperSource, 'Content-Disposition: inline; filename="')
    ...
    'public file endpoints enforce HTTP methods' => str_contains($readOnlyMediaFileSource, 'requireReadOnlyHttpMethod();')
        ...
        && str_contains($readOnlyMediaThumbSource, 'sendInlineStoredFileResponse(')
        && str_contains($readOnlyMediaThumbSource, 'sendFileDownloadNotFound();')
        && !str_contains($readOnlyMediaThumbSource, 'http_response_code(404)')
        ...
        && str_contains($readOnlyGalleryImageSource, 'sendInlineStoredFileResponse(')
        && str_contains($readOnlyGalleryImageSource, 'sendFileDownloadNotFound();')
        && !str_contains($readOnlyGalleryImageSource, 'http_response_code(404)')
        && str_contains($readOnlyPlacesImageSource, '$isHeadRequest = requireReadOnlyHttpMethod();')
        && str_contains($readOnlyPlacesImageSource, 'sendInlineStoredFileResponse(')
        && str_contains($readOnlyPlacesImageSource, 'sendFileDownloadNotFound();')
        && !str_contains($readOnlyPlacesImageSource, 'http_response_code(404)')
        && str_contains($readOnlyPodcastAudioSource, '$isHeadRequest = requireReadOnlyHttpMethod();')
        && str_contains($readOnlyPodcastAudioSource, 'sendFileDownloadNotFound();')
        && str_contains($readOnlyPodcastAudioSource, 'sendNoSniffHeader();')
        && !str_contains($readOnlyPodcastAudioSource, 'http_response_code(404)')
        && str_contains($readOnlyPodcastImageSource, '$isHeadRequest = requireReadOnlyHttpMethod();')
        && str_contains($readOnlyPodcastImageSource, 'sendInlineStoredFileResponse(')
        && str_contains($readOnlyPodcastImageSource, 'sendFileDownloadNotFound();')
        && !str_contains($readOnlyPodcastImageSource, 'http_response_code(404)')
    ...
    'admin JSON endpoints use shared no-store headers' => str_contains($authSource, 'function sendAdminJsonHeaders')
        ...
        && str_contains($adminContentLockRefreshSource, 'sendAdminJsonHeaders();')
    ...
    'admin downloadable exports send safe headers' => str_contains($authSource, 'function sendAdminDownloadHeaders')
        && str_contains($authSource, 'sendAdminNoStoreHeaders();')
        && str_contains($authSource, 'sendNoSniffHeader();')
        && str_contains($fileDownloadHelperSource, 'function sendAdminAttachmentHeaders(')
        && str_contains($fileDownloadHelperSource, 'storedFileContentDisposition(')
        && str_contains($fileDownloadHelperSource, "'attachment',")
        && str_contains($fileDownloadHelperSource, "safeDownloadName(\$downloadName, 'download')")
        && str_contains($fileDownloadHelperSource, 'sendAdminDownloadHeaders();')
        && str_contains($adminExportSource, 'sendAdminAttachmentHeaders(')
        ...
        && !str_contains($adminFormSubmissionFileSource, 'Content-Disposition: attachment; filename=')
        && !str_contains($adminFormSubmissionsSource, 'Content-Disposition: attachment; filename=')
        && !str_contains($adminBackupSource, 'Content-Disposition: attachment; filename=')
        && !str_contains($adminGalleryExportZipSource, 'Content-Disposition: attachment; filename=')
        && !str_contains($adminThemesSource, 'Content-Disposition: attachment; filename='),
    ...
    'shared helper recoverable failures use structured logs' => !str_contains($uiSource, 'error_log(')
        ...
        && str_contains($mediaLibrarySource, 'function mediaLogFilesystemFailure')
        && str_contains($mediaLibrarySource, "koraLog('warning', 'media filesystem operation failed'")
    ...
    'file operation logs avoid raw filesystem paths' => !str_contains($fileDownloadHelperSource, 'error_log(')
        ...
        && !str_contains($mediaLibrarySource, '@unlink(')
        && !str_contains($mediaLibrarySource, '@rename(')
        && !str_contains($mediaLibrarySource, '@copy(')
        && !str_contains($themeSource, 'error_log(')
        && !str_contains($presentationSource, 'error_log(')
...
foreach ($contentSecurityPolicyProbe['headers'] as $securityHeader) {
    ...
    if (str_starts_with($normalizedHeader, 'x-permitted-cross-domain-policies:')) {
        ...
        if (!str_contains($normalizedHeader, 'none')) {
            $contentSecurityPolicyIssues[] = 'public X-Permitted-Cross-Domain-Policies header is not set to none';
        }
        continue;
    }
    if (str_starts_with($normalizedHeader, 'cross-origin-opener-policy:')) {
...
$themeFoodOrderViewSource = (string)file_get_contents(dirname(__DIR__) . '/themes/default/views/modules/food-order.php');
...
foreach (['food-structured-menu', 'aria-labelledby="<?= h($foodStructuredMenuId) ?>"', 'food-menu-item__price', 'food-menu-item__image', 'food-menu-section__serving', 'food-menu-section__badge', 'food-menu-item__nutrition', 'food-allergen-legend', 'Alergeny:'] as $foodStructuredViewFragment) {
...
foreach (['foodServingDateQueryParams', 'serving_date_filter_active', 'Poptat objednávku'] as $foodDailyOrderViewFragment) {
    if (!str_contains($themeFoodIndexViewSource, $foodDailyOrderViewFragment) || !str_contains($themeFoodCardViewSource, $foodDailyOrderViewFragment)) {
        $themeLayoutIssues[] = 'food public views are missing daily/order fragment: ' . $foodDailyOrderViewFragment;
    }
}
foreach (['csrf_token', 'honeypotField()', '<fieldset', '<legend>Vybrané položky</legend>', '<legend>Kontaktní údaje</legend>', 'aria-describedby', 'captcha', 'food-order-items-help'] as $foodOrderViewFragment) {
    if (!str_contains($themeFoodOrderViewSource, $foodOrderViewFragment)) {
        $themeLayoutIssues[] = 'food order view is missing accessible guarded form fragment: ' . $foodOrderViewFragment;
    }
}
...
foreach ([
    'layout visible search landmark' => [
        ...
        'fragments' => [
            ...
            'aria-controls="admin-command-dialog" aria-expanded="false"',
        ],
    ],
