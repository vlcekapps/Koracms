    ...
    if ($slug !== '') {
        $stmt = $pdo->prepare(
            "SELECT a.*, c.name AS category, c.id AS category_id, c.slug AS category_slug, b.slug AS blog_slug,
             ...
             LEFT JOIN cms_blogs b ON b.id = a.blog_id
             LEFT JOIN cms_users u ON u.id = a.author_id
             WHERE a.slug = ? AND a.deleted_at IS NULL AND a.status = 'published' AND (a.publish_at IS NULL OR a.publish_at <= NOW()) AND (a.unpublish_at IS NULL OR a.unpublish_at > NOW())"
        );
        $stmt->execute([$slug]);
    } else {
        $stmt = $pdo->prepare(
            "SELECT a.*, c.name AS category, c.id AS category_id, c.slug AS category_slug, b.slug AS blog_slug,
             ...
             LEFT JOIN cms_blogs b ON b.id = a.blog_id
             LEFT JOIN cms_users u ON u.id = a.author_id
             WHERE a.id = ? AND a.deleted_at IS NULL AND a.status = 'published' AND (a.publish_at IS NULL OR a.publish_at <= NOW()) AND (a.unpublish_at IS NULL OR a.unpublish_at > NOW())"
        );
        $stmt->execute([$id]);
    }
}
