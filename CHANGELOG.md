...
- **CI a vývojové kontroly – GitHub Actions používá `actions/checkout@v6` a PHPStan nově pokrývá další stabilní helpery** – základní CI už nemá varování kvůli Node 20 checkout akci a statická analýza má bezpečný bootstrap bez DB/session side efektů; zahrnuje také `lib/filedownloads.php`, `lib/pagination.php` a `lib/totp.php`.
