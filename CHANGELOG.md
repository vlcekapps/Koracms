...
- **Rezervace v administraci** – přehled rezervací, ruční vytvoření rezervace a detail rezervace používají sdílené admin CSS třídy a atribut `hidden` místo lokálního `<style>` bloku, lokálních `style` atributů nebo JS mutací `element.style`; runtime audit hlídá návrat starého vzoru.
