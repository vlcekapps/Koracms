...
- **Správa widgetů v administraci** – přehled zón, tlačítka, dialog nastavení, dynamická pole a drag stav používají nonce CSS třídy a `hidden` místo lokálních `style` atributů nebo JS mutací `element.style`; runtime audit hlídá, aby se inline styly do obrazovky nevracely.
