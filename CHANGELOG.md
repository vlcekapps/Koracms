...
- **Odkazy otevírané v novém okně v administraci** – obsahové přehledy a editační obrazovky administrace nově u veřejných náhledů, RSS feedů, médií, příloh, GitHub issue, externích webů a dalších odkazů doplňují přístupný název s informací o otevření v novém okně; runtime audit hlídá celý `admin/*.php`, aby se `target="_blank"` nevrátil bez sdíleného `newWindowLinkLabel()`.
- **Odkazy otevírané v novém okně v blogové administraci** – odkazy na veřejný blog, RSS feed, náhled článku, aktuální náhledový obrázek a veřejné blogové stránky nově v blogových administračních obrazovkách oznamují čtečkám obrazovky otevření v novém okně; runtime audit hlídá, aby se `target="_blank"` v blogovém admin workflow nevrátil bez `newWindowLinkLabel()`.
