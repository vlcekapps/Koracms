...
- **Theme view audit pro default šablonu** – `composer ci:basic` nově spouští `build/theme_view_audit.php`, který hlídá, že PHP view soubory default šablony zůstávají prezentační vrstvou bez přímého request inputu, databázových operací, souborových side effectů, inline stylů, inline event handlerů a script tagů bez CSP nonce; nově kontroluje i statická duplicitní `id` a statické odkazy `aria-labelledby` / `aria-describedby` na neexistující prvky.
- **Self-test theme view auditu** – `composer ci:basic` nově spouští i `build/theme_view_audit_selftest.php`, který na dočasných fixtures ověřuje, že theme view audit umí projít čistou šablonu a opravdu selže na request inputu, databázi, inline stylech, skriptu bez CSP nonce, duplicitním statickém `id` a chybějící statické ARIA vazbě.
