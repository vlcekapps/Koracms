...
Mobilní baseline administrace hlídá runtime sekce `admin_mobile_reflow_guardrails`. Sdílený admin stylesheet na malé šířce skládá navigaci nad obsah, zmenšuje padding hlavní části, nechává datové tabulky scrollovat lokálně přes `.table-responsive`, skládá media/Form Builder/statistics gridy do jednoho sloupce a dorovnává malé řadicí ovladače na minimální target size. Browser ověření při 320 px pokrývá media, widgets, statistics, Form Builder a přehled formulářů; statistiky a formuláře mají wrappery tabulek tak, aby neroztahovaly celý viewport.
