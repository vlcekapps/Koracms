...
| **Appmarket** | Katalog softwaru pro Windows, Linux, macOS a Android, více platforem, historie verzí, changelogy, systémové požadavky a soukromé úložiště instalačních balíčků |
| **Jídelní lístek** | Karty jídel a nápojů s platností, denními nabídkami, strukturovanými položkami, cenovými a porčními variantami, alergeny, nutričními údaji, obrázky, veřejnými filtry, objednávkovými poptávkami, archivem, hledáním a revizemi |
...
Modul **Appmarket** je katalog softwaru pro Windows, Linux, macOS, Android a další platformy, bez omezení počtu aplikací. Každá aplikace má popis, obrázky, licenci a veřejnou stránku `/aplikace/{slug}`. Akce **Aktualizovat verzi** nabídne číslo verze, platformu, changelog, instalační soubor a požadavky na systém. Nové vydání ponechá starší verze v historii; **Upravit verzi** umožní opravit údaje konkrétního ručního vydání nebo nahradit jeho soubor. Více platforem může používat stejné číslo verze. Veřejný detail nabídne poslední stabilní vydání pro každou platformu (nebo beta, pokud stabilní chybí).

Podporované jsou `.zip`, `.exe`, `.msi`, `.msix`, `.msixbundle`, `.rpm`, `.deb`, `.dmg`, `.pkg`, `.tar`, `.tar.gz`, `.tar.xz`, `.tar.bz2`, `.tar.zst`, `.tar.lz`, `.tar.lzma`, `.tgz`, `.txz`, `.tbz`, `.tbz2`, `.tzst`, `.7z`, `.AppImage`, `.flatpak`, `.flatpakref`, `.snap`, `.apk`, `.aab`, `.run` a `.sh`. Upload respektuje nastavený limit CMS i limity PHP/hostingu. Balíčky se nerozbalují ani nespouštějí, ukládají se mimo webroot a download ověřuje velikost a SHA-256 s podporou `GET`, `HEAD` a HTTP Range. CMS neprovádí antivirovou kontrolu; za podpisy, certifikáty, licenci a důvěryhodnost softwaru odpovídá správce. Běžné vydání nepotřebuje Android SDK ani publisher klíč.

Původní Android update API V1/V2/V3 a `tools/appmarket-publish.ps1` zůstávají jako oddělená kompatibilní cesta pro dříve zavedené podepsané aktualizace. Obecné ruční vydání se do těchto API nikdy nenabídne, ani pokud jde o APK. V této starší cestě zůstávají podpisové kontroly, OpenSSL, ABI a anonymní rollout zachované; podepsaný publisher posílá koncept přes `/api/appmarket/v1/releases`. Ruční upload v katalogu již publisher ZIP neinterpretuje a zpřístupnil by jej jen jako obyčejný archiv.

Po aktualizaci kódu spusťte `migrate.php` nad zálohovanou databází. Původní APK data a veřejné URL zůstanou zachované. Export/import zahrnuje katalogová metadata, nikoli soubory a přístupové tokeny; importované verze jsou koncepty bez souboru. Zveřejnění a úpravy veřejného vydání nadále vyžadují superadmina.

Klient V3 posílá `package_id`, aktuální `version_code`, `sdk_int`, kanál `stable` nebo `beta`, primární ABI zařízení a volitelný `rollout_bucket` 0–99. Aplikace si bucket náhodně vytvoří jednou a uchová jej lokálně; server z něj nedokáže odvodit identitu zařízení. Beta klient může obdržet beta i stabilní vydání, stable klient pouze stabilní. Vydání bez důvěryhodných ABI metadat se přes V3 nenabídne odhadem.
...
Každá strukturovaná položka může mít cenové a porční varianty, například malou a velkou pizzu nebo nápoj 0,3 a 0,5 l. Varianta má vlastní cenu, měnu, poznámku a dostupnost; pokud varianty existují, základní cena položky zůstává jen fallbackem a veřejný lístek, JSON-LD i objednávková poptávka používají varianty. Lístek může nabídnout osobní vyzvednutí, konzumaci na místě nebo doručení a volitelně vyžadovat požadovaný budoucí termín. Server vždy znovu načte povolenou variantu i cenu z databáze a do objednávky uloží snapshot názvu, porce a ceny, takže request nemůže podstrčit vlastní částku. Přístupnost tohoto workflow popisuje modulová příloha [docs/accessibility/modules/food.md](docs/accessibility/modules/food.md).
...
Export ZIP balíčku šablony před stažením ukazuje review vybrané šablony, upozorní na přenos vizuální konfigurace a statických assetů a vyžaduje potvrzení oprávnění ke stažení.
...
| `[video src="/uploads/media/video.mp4" captions="/uploads/media/video.cs.vtt" srclang="cs" descriptions="/uploads/media/video-popis.cs.vtt"][/video]` | HTML5 video s WebVTT titulky a zvukovým popisem |
...
- bezpečné předvyplnění nebo opětovné použití kontaktních údajů přes `currentUserContactDefaults()` ve veřejném kontaktu, Food objednávce, veřejném chatu, blogových komentářích a přihlášených veřejných rezervacích
...
- před stažením JSON exportu se zobrazí review citlivosti exportovaných dat a server vyžaduje potvrzení oprávnění; bez potvrzení se soubor nestáhne
...
- CSV export výsledků anket používá stejný princip pro agregovaná data: kontrolní stránka ukáže téma ankety, počty hlasujících a odpovědí, stažení vyžaduje `confirm_poll_results_csv_export` a server bez potvrzení neodešle soubor ani nezapíše audit log
- CSV export audit logu používá stejný princip pro provozní a bezpečnostní záznamy administrace: kontrolní stránka ukáže aktuální filtr, počet záznamů a citlivost detailů akcí, stažení vyžaduje `confirm_audit_log_csv_export` a server bez potvrzení neodešle soubor ani nezapíše audit log
...
Obnova lokálního konceptu zachová rozepsaný obsah a běžné datové volby, ale nikdy nepřevezme staré potvrzení kritické akce (`confirm_*` nebo `data-autosave-confirmation`). Po obnově takového formuláře oznámí nutnost nové kontroly; průběžné ukládání přitom aktuální potvrzení uživatele neruší. Ochrana platí i pro starší recovery koncepty.
...
- accessibility conformance audit přes `build/accessibility_conformance_audit.php` a jeho negativní self-test, které hlídají úplnost 55 WCAG 2.2 A/AA kritérií, shodu matice s ACR, traceability, aktuální metadata a povinné impact review s automatizovaným důkazem u přístupnostně citlivého diffu
...
server {
    ...
    location = /api/appmarket/v2/update { rewrite ^ /appmarket/update_v2.php last; }
    location = /api/appmarket/v3/update { rewrite ^ /appmarket/update_v3.php last; }
...
- [docs/module-proposal-template.md](docs/module-proposal-template.md) – vyplnitelná šablona návrhu nového modulu nebo větší modulové funkce
...
- [docs/accessibility/a11y-impact-decisions.md](docs/accessibility/a11y-impact-decisions.md) – auditní deník přístupnostně citlivých změn bez změny ACR stavu
...
- [docs/accessibility/modules/blog.md](docs/accessibility/modules/blog.md) – modulová příloha pro Blog: veřejná i administrační workflow, důkazy, ruční scénáře a backlog
