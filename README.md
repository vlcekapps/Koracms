...
| **Appmarket** | Vlastní katalog Android aplikací s historií vydání, soukromým úložištěm APK, kontrolou podpisu, offline publisher balíčky a anonymním update API podle kompatibility zařízení |
...
Modul **Appmarket** je specializovaný na distribuci produkčních Android APK a aktualizace aplikací. Není omezený na konkrétní projekty ani počet aplikací: každá aplikace má stabilní `applicationId`, automaticky generovatelný slug, veřejnou stránku `/aplikace/{slug}` a historii vydání. Kompatibilní API V1 zůstává na `/api/appmarket/v1/update`; nové `/api/appmarket/v2/update` navíc přijímá Android `sdk_int`, vybírá jen kompatibilní vydání a vrací naléhavost, hranici povinné aktualizace i změny oprávnění. Ani jedna verze API nepoužívá identifikátor zařízení. APK se ukládají do privátního úložiště mimo webroot a stahují se přes kontrolovaný endpoint s ověřením velikosti a SHA-256, podporou `GET`/`HEAD` a jednoho HTTP Range.

Obecný lokální publisher `tools/appmarket-publish.ps1` je součástí release ZIPu. V čistém Android Git projektu najde produkční release APK, ověří jej lokálními nástroji `apkanalyzer` a `apksigner` a podepíše přesný manifest samostatným RSA-3072 publisher klíčem. Koncept může odeslat přímo na `/api/appmarket/v1/releases`, nebo vytvořit přenosný `.kora-app-release.zip` s APK, manifestem, podpisem a seznamem změn pro ruční upload v administraci. Android SDK je potřeba pouze na důvěryhodném lokálním počítači. Hosting žádné Android nástroje nepotřebuje; pro ověření podpisu vyžaduje PHP OpenSSL a pro ruční ZIP také rozšíření ZIP. Soukromý zdrojový repozitář, Android keystore ani privátní publisher klíč se do CMS neposílají.

Server žádné vydání nepublikuje automaticky. Správce nejprve schválí podpisový certifikát APK a na samostatné kontrolní obrazovce porovná identitu, podpis, změnu SDK, nová či odebraná oprávnění, hash a bezpečně vykreslený seznam změn. Při publikaci zvolí běžnou, důležitou nebo kritickou aktualizaci a může určit `versionCode`, pod kterým už má klient aktualizaci vyžadovat. Zneplatnění certifikátu okamžitě stáhne všechna jeho veřejná vydání.
