...
- runtime guardrail pro administraci, který hlídá, že odkazy otevírané v novém okně v `admin/*.php` používají sdílený `newWindowLinkLabel()`, dynamicky vytvořené odkazy mají bezpečné `rel` a přístupný popis a `window.open()` používá `noopener,noreferrer`
