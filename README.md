...
- runtime guardrail pro administraci, který hlídá, že odkazy otevírané v novém okně v `admin/*.php` používají sdílený `newWindowLinkLabel()` a `rel="noopener noreferrer"`, dynamicky vytvořené odkazy mají stejnou ochranu i přístupný popis a `window.open()` používá `noopener,noreferrer`; theme audit u veřejných šablon navíc uznává i nový vzor se skrytým textem přímo uvnitř odkazu
