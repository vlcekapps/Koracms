...
Soubor `robots.txt` je generovaný přes `robots.php`, podporuje jen `GET` a `HEAD`, zakazuje indexaci administrace a citlivých upload adresářů a odkazuje na aktuální sitemapu. Stejné čtecí omezení metod používají také XML sitemapa, globální, blogové i podcastové RSS feedy, ICS export událostí a veřejné souborové/media endpointy. U souborů vrací `HEAD` jen hlavičky, bez přenosu těla souboru.
