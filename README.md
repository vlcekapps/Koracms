...
Živý náhled šablony a veřejný rezervační kalendář také používají skutečný textový název: preview banner je napojený na vlastní text v banneru a kalendářová tabulka má skrytý `<caption>`.
...
- statický release package audit, který hlídá, že instalační balíček a source archivy zůstávají bez vývojových nástrojů, lokálních metadat, citlivých konfigurací a uživatelských uploadů
...
- runtime guardrail pro administraci, který hlídá, že odkazy otevírané v novém okně v `admin/*.php` používají sdílený `newWindowLinkLabel()` a `rel="noopener noreferrer"`, dynamicky vytvořené odkazy mají stejnou ochranu i přístupný popis a `window.open()` používá `noopener,noreferrer`
- izolovaný release smoke test, který v dočasném snapshot repozitáři skutečně spustí `build/release.ps1 -DryRun -SkipCi`, ověří čistý git stav po běhu, zkontroluje obsah release ZIPu i checksum a navíc ověří skutečný `git archive` source balíček podle `.gitattributes`; oba artefakty musí obsahovat kritické instalační soubory a nesmí obsahovat `vendor`, `dist`, lokální IDE/Claude metadata, citlivé konfigurace ani uploadovaný obsah mimo `uploads/.htaccess`
