...
- theme view audit pro default šablonu včetně self-testu, který hlídá oddělení prezentační vrstvy od requestu, session/server stavu, runtime času, databáze a souborových side effectů i statická duplicitní `id`, neexistující cíle `aria-labelledby` / `aria-describedby` / `aria-controls`, neplatné statické `label for`, veřejné `<section>`, `<nav>`, `<aside>` a `role="search"` bloky bez `aria-labelledby`, formulářová pole bez labelu nebo ARIA názvu, `<fieldset>` bez `<legend>`, obrázky bez `alt`, iframe bez `title`, tlačítka bez explicitního `type`, veřejné tabulky bez `<caption>` nebo `aria-labelledby` a `target="_blank"` odkazy bez `rel="noopener noreferrer"` nebo bez oznámení nového okna v přístupném názvu
