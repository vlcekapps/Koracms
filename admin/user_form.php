...
if ($id !== null) {
    ...
    $stmt->execute([$id]);
    $account = $stmt->fetch();
    if (!$account) { header('Location: users.php'); exit; }
}
...
if (!empty($formData) && !$account) {
    $account = $formData + ['email'=>'','first_name'=>'','last_name'=>'','nickname'=>''];
} elseif (!empty($formData)) {
    $account = array_merge($account, $formData);
}

adminHeader($account ? 'Upravit spolupracovníka' : 'Nový spolupracovník');
?>
...
<form method="post" action="user_save.php" novalidate>
  ...
  <fieldset>
    ...
    <input type="text" id="nickname" name="nickname" maxlength="100"
           value="<?= h($account['nickname'] ?? '') ?>">
  </fieldset>

  <fieldset style="margin-top:1.5rem;border:1px solid #ccc;padding:.5rem 1rem">
    <legend><?= $account ? 'Změna hesla <small>(ponechte prázdné pro beze změny)</small>' : 'Heslo <span aria-hidden="true">*</span>' ?></legend>
    <label for="new_pass">Heslo (min. 8 znaků)</label>
    <input type="password" id="new_pass" name="new_pass" minlength="8"
           autocomplete="new-password" <?= $account ? '' : 'required aria-required="true"' ?>>
  ...
  <div style="margin-top:1.5rem">
    <button type="submit" class="btn"><?= $account ? 'Uložit' : 'Vytvořit účet' ?></button>
