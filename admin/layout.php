...
function adminEditorFormFields(): array
{
    return [
        ...
        'gallery_photo' => ['title', 'slug', 'alt_text', 'caption', 'description', 'credit', 'license_label', 'license_url', 'taken_at', 'location_label', 'sort_order'],
        'poll' => ['question', 'slug', 'description', 'status', 'vote_mode', 'max_choices', 'results_visibility', 'start_date', 'start_time', 'end_date', 'end_time', 'meta_title', 'meta_description'],
...
function adminRenderContentLockRefreshScript(string $entityType, ?int $entityId): void
{
    ...
    echo '<script nonce="' . h($nonce) . '">'
       ...
       . 'if(live)live.textContent="Přihlášení pravděpodobně vypršelo. Rozepsaný obsah se dál ukládá lokálně; před odesláním se znovu přihlaste.";'
       . '}'
       ...
       . '.then(function(response){if(response.status===401){notifySessionExpired();return null;}return response.ok?response.json():null;})'
...
function adminFooter(): void
{
    ...
    echo '<script src="' . h(BASE_URL . '/admin/assets/command.js?v=' . rawurlencode(KORA_VERSION)) . '" nonce="' . $nonce . '"></script>'
       ...
       . 'return (el.type==="checkbox"||el.type==="radio")&&((el.name||"").indexOf("confirm_")===0||el.hasAttribute("data-autosave-confirmation"));'
       . '}'
       ...
       . 'if(el.name===\'csrf_token\'||isConfirmationField(el))return;'
       . 'd[\'__chk_\'+el.name+\'_\'+el.value]=el.checked?\'1\':\'0\';'
       . '});'
       . 'd._ts=Date.now();'
       . 'return d;'
       . '}'
       . 'function saveDraft(targetKey){try{localStorage.setItem(targetKey,JSON.stringify(gather()));}catch(e){}}'
       . 'function restore(d){'
       ...
       . 'if(el&&!isConfirmationField(el))el.checked=(d[k]===\'1\');'
       . 'return;'
       . '}'
       ...
       . '});'
       . 'var live=document.getElementById("a11y-live");'
       . 'if(needsConfirmation&&live){live.textContent="Koncept byl obnoven. Před odesláním znovu potvrďte kontrolu akce.";}'
       . '}'
       . 'try{'
       . 'var raw=localStorage.getItem(key);'
       . 'var recoveryRaw=localStorage.getItem(recoveryKey);'
       . 'var saved=raw?JSON.parse(raw):null;'
       . 'var recovery=recoveryRaw?JSON.parse(recoveryRaw):null;'
       . 'var fromRecovery=false;'
       . 'if(recovery&&(!saved||((recovery._ts||0)>(saved._ts||0)))){saved=recovery;fromRecovery=true;}'
       . 'if(saved){'
       . 'if(saved._ts&&(Date.now()-saved._ts)<86400000){'
       . 'var banner=document.createElement(\'div\');'
       . 'banner.setAttribute(\'role\',\'status\');'
       . 'banner.className=\'autosave-banner\';'
       . 'var message=fromRecovery?\'Nalezen záložní koncept z posledního odeslání \':\'Nalezen neuložený koncept z \';'
       . 'banner.innerHTML=\'<span>\'+message+new Date(saved._ts).toLocaleString(\'cs-CZ\')+\'.</span>\''
       ...
       . 'btns[0].addEventListener(\'click\',function(){restore(saved);localStorage.removeItem(recoveryKey);banner.remove();});'
       . 'btns[1].addEventListener(\'click\',function(){localStorage.removeItem(key);localStorage.removeItem(recoveryKey);banner.remove();});'
       . 'form.parentNode.insertBefore(banner,form);'
       . '}else{localStorage.removeItem(key);localStorage.removeItem(recoveryKey);}'
       . '}'
       . '}catch(e){}'
       . 'setInterval(function(){saveDraft(key);},30000);'
       . 'form.addEventListener(\'submit\',function(){saveDraft(recoveryKey);});'
       . '})();'
       . '</script>'
       . '<script nonce="' . $nonce . '">'
       . '(function(){'
