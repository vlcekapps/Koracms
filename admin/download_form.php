...
$downloadImageHelpIds = ['download-image-help'];
if ((string)$download['image_url'] !== '') {
    $downloadImageHelpIds[] = 'download-image-current';
}
...
<form method="post" action="download_save.php" enctype="multipart/form-data" novalidate<?= $errorMessage !== '' ? ' aria-describedby="form-error"' : '' ?>>
  ...
  <fieldset>
    ...
    <small id="download-image-help" class="field-help">Volitelné. Hodí se pro detail položky a sdílení odkazu. Nahrajte JPEG, PNG, GIF nebo WebP; SVG a jiné formáty CMS nepřijímá.</small>
    <?php adminRenderFieldError('download_image', $err, $fieldErrorMap, $fieldErrorMessages['download_image'], 'download-image-error'); ?>
