...
$replyConfirmField = 'confirm_chat_reply_send_' . $messageId;
$replyConfirmId = 'confirm-chat-reply-send-' . $messageId;
$replyReviewId = 'chat-reply-review-' . $messageId;
$replyConfirmErrorId = 'confirm-chat-reply-send-' . $messageId . '-error';
$replyFormErrorId = 'chat-reply-form-error';
$replyFlash = adminReplyFlashPull('chat', $messageId);
...
$replySubjectValue = $replyHasFlash
    ...
    : (trim((string)($message['replied_subject'] ?? '')) !== ''
        ? (string)$message['replied_subject']
        : 'Re: zpráva z chatu – ' . getSetting('site_name', 'Kora CMS'));
...
<?php if (!filter_var((string)$message['email'], FILTER_VALIDATE_EMAIL)): ?>
  <p>Tato zpráva neobsahuje platnou e-mailovou adresu, takže na ni nejde odpovědět přímo z administrace.</p>
<?php else: ?>
  <form method="post" action="<?= BASE_URL ?>/admin/chat_reply.php" novalidate<?= $replyHasFormError ? ' aria-describedby="' . h($replyFormErrorId) . '"' : '' ?>>
    <input type="hidden" name="csrf_token" value="<?= h(csrfToken()) ?>">
    <input type="hidden" name="id" value="<?= (int)$message['id'] ?>">
    <input type="hidden" name="redirect" value="<?= h($selfRedirect) ?>">
    <fieldset>
      ...
      <textarea id="reply-message" name="message" rows="6" required aria-required="true"<?= adminFieldAttributes('reply_message', $replyFieldErrors, [], [], 'reply-message-error') ?>><?= h($replyMessageValue) ?></textarea>
    ...
    <fieldset class="admin-fieldset-card admin-fieldset-spaced" aria-describedby="<?= h($replyReviewId) ?>">
      ...
      <p id="<?= h($replyReviewId) ?>" class="field-help field-help--flush">E-mail se po potvrzení nevratně odešle na <strong><?= h((string)$message['email']) ?></strong>. Před odesláním zkontrolujte příjemce, předmět a text; odpověď se také uloží do historie chatové zprávy.</p>
    ...
    <div class="button-row admin-action-row">
      <button type="submit" class="btn" data-confirm="Opravdu odeslat tuto odpověď na <?= h((string)$message['email']) ?>?">Odeslat odpověď</button>
    </div>
  </form>
<?php endif; ?>
