...
| 3.3.3 Error Suggestion | AA | Partially Supports | Veřejná matematická ověřovací otázka v kontaktu, newsletteru, odběru vývěsky, Food objednávkách a Form Builderu už používá sdílenou hlášku s konkrétním návrhem opravy; mnoho dalších formulářů má konkrétní chybové texty, ale systémová inventura všech návrhů oprav chybí. | Field-level errors, runtime `public_error_suggestion_guardrails`, HTTP scénáře pro chybné ověření ve veřejných formulářích. | Dokončit copy pass administračních, custom a méně častých validačních chyb. |
