...
| 3.3.3 Error Suggestion | AA | Partially Supports | Veřejná matematická ověřovací otázka v kontaktu, newsletteru, odběru vývěsky, Food objednávkách a Form Builderu už používá sdílenou hlášku s konkrétním návrhem opravy; administrační URL chyby u míst a podcastů radí použít http/https adresu, doménu bez schématu nebo prázdné volitelné pole. Vybrané administrační e-mailové chyby radí úplnou adresu ve tvaru `jmeno@example.cz`, prázdné volitelné pole nebo jedinečnost přihlašovací adresy. Plánovací datum/čas chyby u článků, stránek, novinek, událostí, anket, rezervačních zdrojů a podcastových epizod radí vybrat platnou hodnotu v poli datum/čas, nechat volitelné pole prázdné, odstranit prázdný řádek nebo opravit pořadí začátku/konce. Mnoho dalších formulářů má konkrétní chybové texty, ale systémová inventura všech návrhů oprav chybí. | Field-level errors, runtime `public_error_suggestion_guardrails`, runtime `admin_field_error_guardrails` pro vybrané URL, e-mailové a datum/čas chyby, HTTP scénáře pro chybné ověření ve veřejných formulářích. | Dokončit copy pass zbývajících administračních, custom a méně častých validačních chyb. |
