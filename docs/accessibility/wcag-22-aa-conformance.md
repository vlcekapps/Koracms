...
| 1.4.10 Reflow | AA | Partially Supports | Veřejná vrstva i administrace mají responzivní baseline; na mobilní šířce se admin navigace skládá nad obsah, hlavní obsah má lokální scroll pro datové tabulky a komplexní gridy spadají do jednoho sloupce. | Theme view audit, runtime smoke a `admin_mobile_reflow_guardrails` pro admin layout, media, widgets, statistics a Form Builder. | Ručně ověřit 320 px / 400 % zoom u hlavních admin tabulek, pickeru, dialogů a formulářů s reálnými daty. |
...
| 2.4.11 Focus Not Obscured (Minimum) | AA | Partially Supports | Skip link a sticky prvky mají offsety; admin mobilní baseline nemá fixed sidebar a command dialog se na malé šířce vejde do viewportu. | Blog TOC scroll offset, layout CSS a `admin_mobile_reflow_guardrails`. | Ručně ověřit sticky header/admin bar, dialogy, anchor skoky a tabulkový scroll při 320 px / 400 % zoomu. |
...
| 2.5.8 Target Size (Minimum) | AA | Partially Supports | Veřejná UI míří na dostatečné cíle; admin tlačítka, mobilní navigace a řadicí ovladače mají sdílené minimální rozměry nebo flex rozložení. | CSS a runtime `admin_mobile_reflow_guardrails`. | Ručně změřit ikonové/řádkové akce v hustých tabulkách a potvrdit cíle v mobilní administraci. |
