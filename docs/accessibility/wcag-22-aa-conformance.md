...
| 1.4.10 Reflow | AA | Partially Supports | Veřejná vrstva i administrace mají responzivní baseline; na mobilní šířce se admin navigace skládá nad obsah, datové tabulky rolují lokálně a komplexní gridy spadají do jednoho sloupce. Statistiky a přehled formulářů mají doplněné responzivní tabulkové wrappery, aby neroztahovaly celou stránku. | Theme view audit, runtime smoke, `admin_mobile_reflow_guardrails` a browser ověření 320 px dne 2026-07-04 pro media, widgets, statistics, Form Builder a přehled formulářů. | Ručně rozšířit 320 px / 400 % průchod na zbylé husté admin moduly, picker, dialogy a dlouhé formuláře. |
...
| 2.4.11 Focus Not Obscured (Minimum) | AA | Partially Supports | Skip link a sticky prvky mají offsety; admin mobilní baseline nemá fixed sidebar, command dialog se na malé šířce vejde do viewportu a focus v mobilní admin navigaci byl ověřený jako viditelný. | Blog TOC scroll offset, layout CSS, `admin_mobile_reflow_guardrails` a browser ověření admin hledání při 320 px dne 2026-07-04. | Ručně ověřit sticky header/admin bar, dialogy, anchor skoky a lokální tabulkový scroll ve zbylých modulech při 320 px / 400 % zoomu. |
...
| 2.5.8 Target Size (Minimum) | AA | Partially Supports | Veřejná UI míří na dostatečné cíle; admin tlačítka, mobilní navigace, řadicí ovladače a ověřené řádkové akce mají sdílené minimální rozměry nebo flex rozložení. | CSS, runtime `admin_mobile_reflow_guardrails` a browser ověření 320 px bez cílů pod 24 px v media, widgets, statistics, Form Builderu a přehledu formulářů. | Ručně změřit ikonové/řádkové akce ve zbylých hustých tabulkách a custom modulech. |
