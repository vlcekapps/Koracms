...
| 1.3.5 Identify Input Purpose | AA | Partially Supports | Mnoho formulářů používá správné typy polí a autocomplete tam, kde je běžné. | Formulářové auditní guardraily. | Doplnit systematickou kontrolu `autocomplete` u auth, kontaktu, objednávek a rezervací. |
...
| 2.1.1 Keyboard | A | Partially Supports | Formuláře, navigace, command centrum a dialogy mají klávesové chování. | Runtime/theme audit a HTTP scénáře. | Ručně projít všechny kritické JS dialogy bez myši. |
| 2.1.2 No Keyboard Trap | A | Supports | Dialogy mají focus trap s návratem fokusu a escape/close chováním. | Picker, widgets, command centrum guardrails. | Ručně ověřit při kombinaci screen reader + browser. |
...
| 2.4.3 Focus Order | A | Partially Supports | Layouty drží DOM pořadí; dialogy řízeně vracejí focus. | Theme guardrails a JS dialogy. | Ručně ověřit admin dashboard, media picker, widgets a dlouhé formuláře. |
...
| 3.3.8 Accessible Authentication (Minimum) | AA | Partially Supports | Login, registrace, žádost o obnovu hesla a 2FA nevyžadují matematickou CAPTCHA ani jiný kognitivní test; registrace a reset request používají CSRF, rate limit a honeypot. | Runtime audit hlídá, že se CAPTCHA nevrátí do registračního ani resetovacího auth flow; login, 2FA, public auth. | Ručně ověřit celé auth flow se správcem hesel, bez kopírování ručně opisovaného kódu a se screen readerem. |
| 4.1.2 Name, Role, Value | A | Supports | Ovládací prvky mají labely/role/stavy, dialogy a landmarky jsou pojmenované. | Theme view audit, runtime audit. | Ručně ověřit JS změny stavů v command centru, pickeru a widget dialogu. |
...
Kora CMS má nadprůměrně silnou přístupnostní kostru: skip linky, heading-backed landmarky, formulářové vazby, dialog focus management, runtime a theme view guardrails. Největší rizika nejsou v jedné fatální chybě, ale v oblastech, které vyžadují ruční ověření: média a titulky, kontrast theme variant, klávesnice u dynamických prvků, target size v husté administraci, timeouts a nová WCAG 2.2 kritéria pro redundant entry a accessible authentication.
