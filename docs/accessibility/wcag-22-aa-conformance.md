...
| 3.3.8 Accessible Authentication (Minimum) | AA | Partially Supports | Login, registrace, žádost o obnovu hesla, tokenový reset a 2FA nevyžadují matematickou CAPTCHA ani jiný kognitivní test; registrace a reset request používají CSRF, rate limit a honeypot. Admin login a 2FA chyby mají text-backed alerty. | Runtime audit a `auth_accessibility_http` hlídají absenci CAPTCHA v auth flow, honeypot, auth autocomplete metadata, 2FA `one-time-code`/numeric pattern a admin/2FA chybové alerty. | Ručně ověřit celé auth flow ve Firefox/NVDA a Chrome keyboard-only se správcem hesel, TOTP kódem, tokenovým resetem a chybovými stavy. |
