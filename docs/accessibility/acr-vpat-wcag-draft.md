...
| 1.4.10 Reflow | Partially Supports | Runtime `admin_mobile_reflow_guardrails` covers the shared admin mobile baseline. Browser verification on 2026-07-04 at 320 px covered media, widgets, statistics, Form Builder, forms overview, comments, contact, chat, reservations, food, downloads, gallery, import screens and the content picker. Dense admin tables now use local responsive wrappers and the admin stylesheet is cache-busted. Broader 320 px / 400 % coverage for long edit forms, custom modules and assistive technology combinations is still required. |
