...
7. Editor blogového článku, statické stránky, FAQ a FAQ kategorií: dlouhá editace, content lock heartbeat, uložení a chybové stavy. U statické stránky ověřit chyby názvu, slugu, přiřazeného blogu, plánovaného publikování a plánovaného zrušení publikace; u FAQ ověřit prázdnou otázku/odpověď a neplatný nebo duplicitní slug; u FAQ kategorií ověřit prázdný název, slug bez použitelného znaku, duplicitní slug a příliš dlouhý meta title. Souhrnný `role="alert"` má upozornit na dotčené pole, pole má mít `aria-invalid`, `aria-describedby` na existující nápovědu/chybu a text má radit konkrétní opravu.
