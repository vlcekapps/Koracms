...
7. Editor blogového článku, správa blogů, blogových kategorií/štítků, sérií článků, externích odkazů blogu, statické stránky, FAQ a FAQ kategorií: dlouhá editace, content lock heartbeat, uložení a chybové stavy. Ve správě blogů ověřit prázdný název, prázdný/rezervovaný/duplicitní slug a zachování rozepsaných hodnot. U editoru článku ověřit chybějící titulek a text, nepoužitelný slug, cizí kategorii, cizí související článek a cizí sérii; u statické stránky ověřit chyby názvu, slugu, přiřazeného blogu, plánovaného publikování a plánovaného zrušení publikace; u blogových kategorií a štítků ověřit prázdný název a duplicitní slug; u série článků prázdný název; u externího odkazu blogu prázdný název a neplatnou URL; u FAQ ověřit prázdnou otázku/odpověď a neplatný nebo duplicitní slug; u FAQ kategorií ověřit prázdný název, slug bez použitelného znaku, duplicitní slug a příliš dlouhý meta title. Souhrnný `role="alert"` má upozornit na dotčené pole, pole má mít `aria-invalid`, `aria-describedby` na existující nápovědu/chybu a text má radit konkrétní opravu.
...
12. Nastavení webu, nastavení modulů, hlavní navigace, přesměrování, import/export a migrace. U hlavní navigace ověřit externí odkaz bez názvu a s neplatnou URL; u přesměrování ověřit prázdnou/neplatnou starou cestu, prázdnou/neplatnou novou cestu, shodné cesty a duplicitní starou cestu. Souhrnný alert má být atomický, dotčené pole má mít `aria-invalid`, `aria-describedby` na existující nápovědu/chybu a odeslané hodnoty se mají po chybě zachovat.
13. U míst a podcastů zadat neplatnou URL do volitelného URL pole a ověřit, že field-level chyba vysvětluje povolený http/https nebo doménový tvar a možnost pole vynechat. U podcastového pořadu navíc ověřit chybějící název, nepoužitelný nebo duplicitní slug, RSS limit a cover obrázek.
