...
7. Editor blogového článku a statické stránky: dlouhá editace, content lock heartbeat, uložení a chybové stavy. U statické stránky ověřit chyby názvu, slugu, přiřazeného blogu, plánovaného publikování a plánovaného zrušení publikace: souhrnný `role="alert"` má upozornit na dotčené pole, pole má mít `aria-invalid`, `aria-describedby` na existující nápovědu/chybu a text má radit konkrétní opravu.
