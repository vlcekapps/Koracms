...
| RC-45 | P2 | Nové RC testy předpokládají nativní celočíselné PDO výsledky a falešně selhávají na PHP 8.0. | build/rate_limit_expiry_selftest.php, build/rc_module_state_selftest.php, build/rc_pdo_fetch_selftest.php | Opraveno: normalizují se jen číselné identifikátory a příznak SQLite schématu; přísné porovnání ostatních polí zůstává. Šest izolovaných sad se v test:rc-core spouští znovu s číselnými řetězci. |
| RC-46 | P2 | Balíček nevynechává agentní metadata, cache formátovače a lokální integrity snapshot. | build/release.ps1, .gitattributes, build/release_smoke.php | Opraveno při přípravě RC: explicitní vyloučení z release ZIPu i source archive, skutečné lokální fixtures ve smoke testu a negativní mutace pravidel. Produkční rozhraní se nemění. |
...
- Následné GitHub běhy nad `058ed5f3` prošly zdrojovými audity i unit testy, ale odhalily odlišnou PDO reprezentaci čísel v nových RC testech (RC-45). Testy expirací, legacy přihlášení, rezervací, provozních stavů, editorů a médií nyní procházejí místně v nativním režimu i s `PDO::ATTR_STRINGIFY_FETCHES`; očekávané aplikační chování se nemění a žádná sada se nevypíná.
- Audit byl původně uzavřen bez vydání. Dne 2026-09-05 správce následně schválil vydání `5.0.0-rc.1` s hostingovým ověřením až po nahrání. GitHub [CI](https://github.com/vlcekapps/Koracms/actions/runs/33983278273) i [Full CI](https://github.com/vlcekapps/Koracms/actions/runs/33983284501) nad `d5a87197` prošly na PHP 8.0, včetně 1539 unit testů, RC sad v obou PDO režimech, čisté instalace a HTTP integrace. Samotné vydání znovu vyžaduje `build/release.ps1 -Version 5.0.0-rc.1 -FullCi`, bez přeskočení CI.
- Aktivace opravy relací/2FA byla výslovně schválena; po nasazení se dosavadní relace znovu přihlásí. Rozpracovaný druhý faktor platí deset minut, nikoli celý editor. Neuzavřený P1 zůstává release blocker; souhlas s RC nemění stavy WCAG/ACR ani neoznačuje hosting za ověřený.
