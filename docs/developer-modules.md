...
- Definice: doplňte module key do centrálního manifestu `coreModuleDefinitions()` v `lib/definitions.php`. Manifest drží labely, výchozí `module_*` hodnotu a kontextové příznaky `profile_managed`, `settings_configurable`, `settings_default`, `public_nav`, `public_nav_path`, `public_nav_order`, `settings_label`, `nav_label`, `widget_label` a `admin_paths`; `install.php`, `migrate.php`, `admin/settings_modules.php`, veřejná navigace, widgetové popisky a testy administračních vstupů se z něj odvozují automaticky. `settings_default` smí být jen `0` nebo `1`; veřejné moduly musí mít cestu začínající `/`, existující PHP entrypoint, odpovídající `isModuleEnabled('...')` bránu, kladné pořadí a musí projít dynamickým HTTP smoke scénářem `public_module_navigation_http` ve vypnutém i zapnutém stavu, neveřejné moduly naopak nechávají veřejnou cestu prázdnou a pořadí `0`.
- Administrace: používejte `requireLogin()` nebo přesnější capability helper, u hlavních administračních přehledů modulu přidejte `requireModuleEnabled()` a jejich URL uveďte v manifestovém `admin_paths`; všechny formulářové, detailové a stav měnící admin soubory modulu zároveň doplňte do `adminRouteModuleRequirement()` v `auth.php`, aby přímá URL nebo POST skončily při vypnutém modulu 403. Stav měnící akce musí mít CSRF tokeny, PRG po POSTu, jasné flash hlášky, audit log a validaci všech ID proti správnému kontextu.
...
- Content picker: pokud nový modul přidává zdroje do `admin/content_reference_picker.php` nebo `admin/content_reference_search.php`, používejte v `isModuleEnabled()` stejný module key jako v `coreModuleDefinitions()`; modulový audit tuto vazbu hlídá spolu s widgety a šablonami. Vypnutý modul se nesmí nabízet v typu pickeru, v nápovědě snippetů ani vracet výsledky z přímého JSON dotazu.
...
- Hlavní administrační přehled modulu musí být uvedený v `admin_paths` a přes `requireModuleEnabled()` vracet 403 při vypnutém modulu; formulářové, detailové a stav měnící endpointy doplňte do `adminRouteModuleRequirement()` a přidejte reprezentativní unit test do sekce `adminRouteModuleRequirement()`. Modulový audit ověřuje, že mapované admin soubory existují, module key je známý, manifestové `admin_paths` jsou v centrální mapě zahrnuté a každý mapovaný soubor má login guard přes `requireLogin()`, `requireSuperAdmin()`, `requireModuleEnabled()` nebo `requireCapability()`. Scénář `admin_disabled_modules_http` hlídá přímé URL z manifestu automaticky a zároveň ověřuje, že stav měnící admin endpoint vypnutého modulu vrací 403 a nic neuloží.
