...
- Modulový zdroj ve veřejném vyhledávání musí mít manifestový záznam `search_result_types`; audit ověřuje, že `search.php` používá manifestové popisky a že každý typ výsledku má odpovídající větev v `resultUrl()`.
- Modulový zdroj v sitemapě musí mít manifestový záznam `sitemap_sections`; audit ověřuje, že `sitemap.php` negatuje nové moduly přes známý module key a že každá logovaná sitemap sekce patří k některému modulu.
