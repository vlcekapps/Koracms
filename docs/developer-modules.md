...
- Administrace: používejte `requireLogin()` nebo přesnější capability helper, u hlavních administračních přehledů modulu přidejte `requireModuleEnabled()` a jejich URL uveďte v manifestovém `admin_paths`, CSRF tokeny pro stav měnící akce, PRG po POSTu, jasné flash hlášky, audit log a validaci všech ID proti správnému kontextu.
...
- Hlavní administrační přehled modulu musí být uvedený v `admin_paths` a přes `requireModuleEnabled()` vracet 403 při vypnutém modulu; scénář `admin_disabled_modules_http` hlídá přímé URL z manifestu automaticky.
