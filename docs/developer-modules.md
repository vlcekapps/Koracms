...
- Administrace: používejte `requireLogin()` nebo přesnější capability helper, u hlavních administračních přehledů modulu přidejte `requireModuleEnabled()` a jejich URL uveďte v manifestovém `admin_paths`; všechny formulářové, detailové a stav měnící admin soubory modulu zároveň doplňte do sdílené mapy `adminRouteModuleRequirements()` v `auth.php`, ze které vychází `adminRouteModuleRequirement()`, aby přímá URL nebo POST skončily při vypnutém modulu 403. Stav měnící akce musí mít CSRF tokeny, PRG po POSTu, jasné flash hlášky, audit log a validaci všech ID proti správnému kontextu.
