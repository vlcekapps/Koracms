...
- Definice: doplňte module key do centrálního manifestu `coreModuleDefinitions()` v `lib/definitions.php`. Manifest drží labely, výchozí `module_*` hodnotu a kontextové příznaky `profile_managed`, `settings_configurable`, `settings_default`, `public_nav`, `public_nav_path`, `public_nav_order`, `settings_label`, `nav_label` a `widget_label`; `install.php`, `migrate.php`, `admin/settings_modules.php`, veřejná navigace a widgetové popisky se z něj odvozují automaticky. `settings_default` smí být jen `0` nebo `1`; veřejné moduly musí mít cestu začínající `/`, existující PHP entrypoint, odpovídající `isModuleEnabled('...')` bránu, kladné pořadí a musí projít dynamickým HTTP smoke scénářem `public_module_navigation_http` ve vypnutém i zapnutém stavu, neveřejné moduly naopak nechávají veřejnou cestu prázdnou a pořadí `0`.
- Administrace: používejte `requireLogin()` nebo přesnější capability helper, u hlavních administračních přehledů modulu přidejte `requireModuleEnabled()`, CSRF tokeny pro stav měnící akce, PRG po POSTu, jasné flash hlášky, audit log a validaci všech ID proti správnému kontextu.
...
- Hlavní administrační přehled modulu musí přes `requireModuleEnabled()` vracet 403 při vypnutém modulu; scénář `admin_disabled_modules_http` hlídá přímé URL pro existující hlavní modulové obrazovky.
