...
- Hlavní administrační přehled modulu musí být uvedený v `admin_paths` a přes `requireModuleEnabled()` vracet 403 při vypnutém modulu; scénář `admin_disabled_modules_http` hlídá přímé URL z manifestu automaticky a zároveň ověřuje, že stav měnící admin endpoint vypnutého modulu vrací 403 a nic neuloží.
