...
- Nový veřejně navigovatelný modul zahrňte do `coreModuleDefinitions()`; společný HTTP scénář `public_module_navigation_http` pak automaticky ověří jeho manifestovou cestu, HTML odpověď a stav zapnutého modulu. Nový samostatný veřejný endpoint zahrňte do HTTP integrace také tehdy, pokud má vlastní routu, hlavičky, redirect, souborovou odpověď nebo stav měnící workflow.
