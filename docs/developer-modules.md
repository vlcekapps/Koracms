...
- Hlavní administrační přehled modulu musí být uvedený v `admin_paths` a přes `requireModuleEnabled()` vracet 403 při vypnutém modulu; formulářové, detailové a stav měnící endpointy doplňte do `adminRouteModuleRequirement()` a přidejte reprezentativní unit test do sekce `adminRouteModuleRequirement()`. Scénář `admin_disabled_modules_http` hlídá přímé URL z manifestu automaticky a zároveň ověřuje, že stav měnící admin endpoint vypnutého modulu vrací 403 a nic neuloží.
