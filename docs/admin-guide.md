...
- `composer format:fix` umí stejnou úzkou sadu helperů lokálně dorovnat do PSR-12 bez zásahu do širšího historického kódu; momentálně pokrývá lint/bootstrap helpery a první stabilní várku sdílených knihoven (`backup`, `comments`, `content`, `definitions`, `filedownloads`, `github`, `mail`, `messages`, `pagination`, `theme`, `totp`, `uploads`, `widgets`)
- `composer analyse:strict` už na levelu 6 vedle základních helperů pokrývá i `comments`, `content`, `definitions`, `github`, `mail`, `messages`, `stats`, `theme`, `ui` a `widgets`; ta část kódu proto nově drží přesnější array kontrakty i bez baseline a bez ignore pravidel
