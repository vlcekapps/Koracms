...
Smazání podcastové kapitoly, osoby nebo odkazu na platformu má vlastní review text a potvrzovací checkbox. Server nepotvrzený POST odmítne před změnou dat i audit logu, vrátí správce ke konkrétní položce a naváže textový atomický alert i lokální chybu na checkbox. Potvrzené smazání odstraní jen zvolenou vazbu: zdrojové audio a ostatní kapitoly, pořad či epizoda a externí profily nebo obsah na cílové platformě zůstávají zachované.
...
- individuální i hromadné odebrání článku je vratný přesun do Koše, nikoli fyzické smazání
...
Řádkové `Přesunout do Koše` má vlastní checkbox pro konkrétní článek. Bez něj server vrátí textový alert a chybu přímo u potvrzení, aniž změní článek, vazby, soubory nebo audit log. Po potvrzení se běžný publikovaný detail i dříve vytvořená preview URL skryjí textovou 404 odpovědí a článek se objeví v Koši; obnova vrátí obě URL, obrázek, komentáře, štítky, série, související články, revize i redirecty. Samostatně potvrzené trvalé smazání v Koši pod zámkem odstraní databázová data a po commitu původní obrázek, miniaturu, WebP i responsive varianty.
...
Upload souboru není povinný, pokud je vyplněný bezpečný externí odkaz ve tvaru `http://`/`https://` adresy nebo domény bez schématu, kterou CMS uloží jako `https://`. Hodí se to pro software a dokumenty hostované mimo CMS, například na GitHub Releases nebo jiné oficiální stránce projektu. Pokud vyplníte lokální soubor i externí odkaz, veřejný výpis i detail nabídnou obě akce. Veřejné otevření externího zdroje prochází interním read-only endpointem, který cíl vždy načte z databáze a nepřijímá cílovou URL z požadavku.
...
- Přehled ukazuje odděleně počet lokálních stažení a otevření externího zdroje spolu s praktickými metadaty, takže správce nemusí otevírat každou položku zvlášť.
...
- Lokální stažení se započítá až po ověření, že soubor existuje a je čitelný; chybějící soubor čítač nezvýší.
- Otevření externího zdroje se počítá odděleně. Jde o klik a přesměrování z Kora CMS, nikoli o potvrzení dokončeného stažení na vzdáleném webu. Veřejný odkaz používá `nofollow`, aby jej běžné vyhledávací roboty zbytečně neotevíraly; jiné automatizované požadavky však nelze zcela vyloučit.
- Detailní administrační statistiky zobrazují dosavadní lokální stažení, externí otevření a jejich součet u aktuálně zveřejněných položek, odděleně od návštěv detailu položky za zvolené období.
...
Publisher je obecný PowerShell skript pro libovolný Android Git projekt a je součástí instalačního ZIPu spolu s podpisovým helperem `tools/appmarket-attest.php`. Vyžaduje lokální PHP s OpenSSL, čistý pracovní strom, produkční release APK a Android SDK nástroje `apkanalyzer` a `apksigner`. Odmítne debug/QA/androidTest, nepodepsané, nezarovnané nebo debug certifikátem podepsané artefakty. Z APK navíc načte seznam nativních ABI a zahrne jej do podepsaného manifestu V3. APK i případný soubor s poznámkami k vydání musí ležet uvnitř ověřovaného repozitáře.

Nejprve mimo zdrojový repozitář aplikace vytvořte samostatný RSA publisher klíč:

```powershell
php .\tools\appmarket-attest.php generate `
    'C:\bezpecne-klice\publisher-private.pem' `
    'C:\bezpecne-klice\publisher-public.pem'
```

V administraci **Publikační tokeny** vložte obsah `publisher-public.pem`. CMS ukáže bearer token pouze jednou. Obě lokální hodnoty pak předejte jen procesními proměnnými:

```powershell
$env:KORA_APPMARKET_TOKEN = 'token-zobrazeny-pri-vytvoreni'
$env:KORA_APPMARKET_SIGNING_KEY = 'C:\bezpecne-klice\publisher-private.pem'
...
Publisher odešle APK, přesně podepsaný manifest a poznámky k vydání, ale na serveru vždy vznikne pouze `Koncept`. Hosting bez Android SDK ověří přes PHP OpenSSL, že manifest podepsal klíč svázaný s použitým tokenem, že není starší než 24 hodin a že velikost, SHA-256 i seznam změn odpovídají uploadu. Pokud má server Android SDK, provede navíc vlastní analýzu APK. Přibalený Apache `.htaccess` zachovává Bearer hlavičku i pod FastCGI; u Nginx musí PHP location předávat `fastcgi_param HTTP_AUTHORIZATION $http_authorization`. Publisher nemůže sám schválit certifikát ani vydání zveřejnit. Token je po vytvoření zobrazený jen jednou, v databázi se ukládá pouze jeho hash a správce jej může kdykoli odvolat. Starší tokeny vytvořené před zavedením publisher klíčů je nutné nahradit. Zdrojový Git repozitář může zůstat soukromý; uživatelé získají výsledné APK, nikoli zdrojové kódy.

Pokud hosting nepřijímá přímé API volání nebo chcete vydání přenést ručně, token není na příkazové řádce lokálního publisheru potřeba. Veřejný publisher klíč ale musí být v administraci stále svázaný s aktivním tokenem, aby měl server čím podpis ověřit. Lokálně vytvořte přenosný balíček:
...
.\tools\appmarket-publish.ps1 `
    -ProjectPath 'C:\cesta\k\android-projektu' `
    -ReleaseNotesPath 'CHANGELOG.md' `
    -OutputBundlePath 'C:\vydani\moje-aplikace.kora-app-release.zip' `
    -SkipUpload
Remove-Item Env:KORA_APPMARKET_SIGNING_KEY
```
...
Pro nové produkční klienty je určen kontrakt V2:

```text
GET /api/appmarket/v2/update?package_id=cz.example.mojeaplikace&version_code=12&sdk_int=35
```

V2 vybere pouze vydání kompatibilní s uvedenou verzí Androidu. Vedle základních údajů vrací `update_priority`, `update_required`, `required_below_version_code` a změny oprávnění proti známé aktuální verzi. Klient má povinnou aktualizaci zobrazit jen tehdy, když je `update_required` skutečně `true`; běžné a důležité aktualizace mohou zůstat odmítnutelné podle návrhu konkrétní aplikace. API V1 zůstává zachované pro starší klienty.

Pro nové klienty s beta kanálem, kontrolou ABI nebo postupným nasazením použijte V3:

```text
GET /api/appmarket/v3/update?package_id=cz.example.mojeaplikace&version_code=12&sdk_int=35&channel=stable&abi=arm64-v8a&rollout_bucket=37
```

`channel` může být `stable` nebo `beta`; beta klient smí obdržet stabilní i beta vydání, stabilní klient jen stabilní. `abi` je primární ABI zařízení. `rollout_bucket` je číslo 0–99, které si aplikace náhodně vytvoří při prvním spuštění a uchová je lokálně. Není to hash Android ID, e-mailu ani jiného identifikátoru a server podle něj nedokáže návštěvníka identifikovat. U 25% rolloutu dostanou vydání buckety 0–24. Hodnota rolloutu 0 aktualizaci pozastaví, 100 ji nabídne všem kompatibilním klientům. Vydání bez důvěryhodných ABI metadat se přes V3 nenabídne odhadem. API V1 a V2 zůstávají konzervativní a vracejí jen stabilní vydání se 100% rolloutem.
...
U Míst je vratný i řádkový a hromadný přesun do Koše. Před odesláním správce kontroluje zachování obrázku, revizí, redirectů a vazeb událostí a server vyžaduje `confirm_place_delete_<id>` nebo `confirm_place_bulk_delete`. Potvrzený přesun nastaví v transakci pouze `deleted_at`; místo zmizí z veřejného webu, administračních přehledů, nových výběrů událostí a veřejné prezentace existujících událostí. Interní vazba existující události zůstane zachovaná a její editor ji označí jako položku v Koši, takže obnovení místa vrátí veřejný vztah bez další editace. Potvrzený trvalý výmaz v Koši odpojí události, smaže revize, redirecty a databázový řádek a po commitu odstraní obrázek. JSON export/import zachovává `deleted_at`.
...
Přehled blogových komentářů má vlastní obdobný pattern. Individuální trvalé smazání vyžaduje checkbox `confirm_comment_delete_<id>`, hromadné trvalé smazání `confirm_comment_bulk_delete`; review upozorňuje na odstranění textu, údajů autora a stavu moderace při zachování článku. Bez potvrzení se formulář vrátí na stejný filtr a hledání s textovým alertem a chybou u checkboxu, komentáře ani audit log se nezmění. Ochrana platí i pro historický `comment_delete.php`, takže starší přímý POST nemůže potvrzení obejít.

Kontaktní a chatové zprávy používají stejný pattern v přehledu, detailu, hromadné akci i historickém přímém endpointu; samostatné serverové potvrzení vyžaduje také smazání chatové odpovědi. Chybějící checkbox vrátí atomický alert a field-level chybu ke konkrétní položce nebo bulk formuláři bez změny zprávy, odpovědí, chatové historie či audit logu. Potvrzené smazání celé chatové zprávy odstraní zprávu, její odpovědi a historii v jedné transakci. Návrat do detailu nebo filtrovaného přehledu prochází přes `internalRedirectTarget()`.
...
- Při zapnutém modulu Ke stažení obsahují podrobné statistiky také blok `Ke stažení`. Ukazuje dosavadní lokální stažení, otevření externích zdrojů a jejich součet u aktuálně zveřejněných položek; filtr období se na tyto čítače nevztahuje, návštěvy detailů jsou v bloku `Výkon obsahu` a externí otevření neprokazuje dokončené stažení na cílovém webu.
...
- Kontrola integrity v administraci používá sdílené administrační CSS třídy pro stavové panely, výsledky kontroly, akční řádek a informační blok místo lokálního `<style>` bloku nebo `style` atributů. Důvěryhodná baseline se ukládá atomicky do privátního úložiště, její vytvoření i obnova vyžaduje výslovné serverové potvrzení a chyba zápisu nezničí předchozí snapshot. Scan zahrnuje také PHP šablony v `themes/` a PHP či ochranné `.htaccess` soubory v `uploads/`; runtime audit hlídá návrat prezentačních i bezpečnostních regresí.
...
- Akce **Obnovit** u lokálního konceptu zachová obsah a běžné datové volby, ale zruší zaškrtnutí potvrzení kritických akcí v obnovovaném formuláři. Platí to i pro staré koncepty a záložní kopii z posledního odeslání. Je-li potřeba nové potvrzení, čtečka dostane stavové oznámení. Před odesláním proto znovu zkontrolujte dopad a vědomě potvrďte akci; běžné automatické ukládání aktuální zaškrtnutí neruší. **Zahodit** odstraní koncept bez změny aktuálních hodnot formuláře.
...
- `php build/accessibility_conformance_audit.php` kontroluje všech 55 kritérií WCAG 2.2 A/AA, shodu matice s ACR, traceability částečně podporovaných kritérií a povinný accessibility impact review s automatizovaným důkazem u citlivých změn; běží i v `composer ci:module-ready`
