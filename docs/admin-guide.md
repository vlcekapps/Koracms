...
- Citlivé veřejné tokenové a odhlašovací endpointy, například potvrzení e-mailu, potvrzení nebo odhlášení newsletteru, reset hesla a veřejné odhlášení, posílají stejné `no-store` a `noindex` hlavičky. Sociální crawler cache výjimka pro sdílený obsah se u těchto URL výslovně nepoužije.
