...
- Hromadné checkboxy `Vybrat vše` v administračních tabulkách používají skrytý `label` navázaný přes `for` / `id` místo samostatného `aria-label`; runtime audit hlídá zachování těchto popisků i to, aby se starý aria-only vzor nevrátil.
