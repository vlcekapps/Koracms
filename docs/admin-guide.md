...
- Běžné administrační HTML odpovědi včetně loginu, 2FA a potvrzení migrace posílají `Cache-Control: no-store, max-age=0`, `Pragma: no-cache` a `Expires: 0`, aby se citlivé administrační obrazovky zbytečně nevracely z cache po odhlášení nebo na sdíleném počítači.
- Veřejné i administrační odpovědi posílají `Permissions-Policy`, která zakazuje nepoužívaná prohlížečová API jako kameru, mikrofon, geolokaci, platby, USB nebo browsing topics. CMS záměrně neblokuje clipboard ani fullscreen, aby zůstala funkční kopírovací tlačítka a legitimní embedy.
