...
- Sdílené UI helpery v `lib/ui.php` používají pro cookie lištu, veřejný admin bar a klávesnicový fallback řazení CSS třídy a `hidden` místo lokálních `style` atributů nebo JS `element.style` mutací; runtime audit hlídá, aby se starý vzor nevrátil.
- Content/media picker v administraci používá sdílený CSS helper s CSP nonce v `lib/ui.php` místo lokálního `<style>` bloku v `admin/content_reference_picker.php`; runtime audit hlídá návrat lokálních stylů i zachování tříd pro dialog, překryv a výsledky vyhledávání.
...
- Veřejné šablony načítají styly pro skip link, `.sr-only`, cookie lištu a veřejný admin bar ze sdíleného `assets/public-core.css` místo generovaného inline a11y `<style>` helperu; samostatné instalační a migrační obrazovky si ponechávají vlastní fallback, protože nemusejí mít načtené theme assety.
- Přihlašovací obrazovky administrace včetně 2FA používají sdílený CSS helper s CSP nonce v `lib/ui.php` místo duplicitních lokálních `<style>` bloků; runtime audit hlídá návrat lokálních stylů i zachování tříd pro TOTP pole a sekundární akci.
