...
- `composer ci:basic` spustí PHP lint, `composer validate --strict`, repository guardrails audit rezervovaných DB připojovacích proměnných, config sample audit `build/config_sample_audit.php` pro sladění `config.sample.php` s hlavní runtime konfigurací, audit GitHub Actions workflow včetně self-testu připnutých actions a zakázaných write/secrets vzorů, source encoding audit platného UTF-8, mojibake audit typických zkomolených českých znaků, whitespace audit koncových mezer a finálního nového řádku, úzký PSR-12 smoke check přes PHP-CS-Fixer včetně build/test helperů, PHPStan na levelu 6, statický release package audit a unit testy; PHPStan používá bezpečný bootstrap `build/phpstan_bootstrap.php` a symbol scan, takže zná sdílené helpery bez načítání databáze nebo session a hlídá i release/testovací nástroje
