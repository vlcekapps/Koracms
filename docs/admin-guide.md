...
- Sdílený admin stylesheet má mobilní baseline pro 320 px / 400 % scénáře: navigace se skládá nad obsah, hlavní obsah má menší padding a lokální horizontální scroll pro datové tabulky, komplexní gridy v médiích, statistikách, Form Builderu a odpovědích formulářů padají do jednoho sloupce a malé řadicí ovladače mají minimální target size. Runtime audit `admin_mobile_reflow_guardrails` hlídá, aby se tento baseline neztratil.
