...
- Chybějící nebo nepřístupné soubory ze sdílených file endpointů vrací textovou `404` odpověď se stejnými `no-store`, `noindex`, `no-referrer` a `nosniff` hlavičkami, aby se podvržené nebo soukromé souborové URL neukládaly v cache ani indexu.
