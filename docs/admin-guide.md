...
- Stavová potvrzení ve veřejné autentizační, účtové, newsletterové a kontaktní části, například po registraci, obnově hesla, uložení profilu, přihlášení k odběru, odeslání zprávy nebo zrušení rezervace v `Moje rezervace`, používají `role="status"` s `aria-atomic="true"` a vlastním textovým uzlem přes `aria-labelledby`, aby hláška nebyla pro čtečky obrazovky anonymní.
