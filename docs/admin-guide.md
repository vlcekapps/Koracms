...
- Stavová potvrzení ve veřejné autentizační a účtové části, například po registraci, obnově hesla, uložení profilu nebo zrušení rezervace v `Moje rezervace`, používají `role="status"` s `aria-atomic="true"` a vlastním textovým uzlem přes `aria-labelledby`, aby hláška nebyla pro čtečky obrazovky anonymní.
