...
- Provozní JSON endpointy `health.php` a `csp-report.php` posílají přes sdílený helper vedle `Cache-Control: no-store` a `X-Content-Type-Options: nosniff` také `X-Robots-Tag: noindex, nofollow, noarchive` a `Referrer-Policy: no-referrer`, aby monitoring a CSP reporty neputovaly přes cache, index nebo referrer a pravidla se časem nerozjela.
