...
- Interní administrační JSON akce, které mění stav přes AJAX, jsou POST-only. Při jiné metodě vrací `405` s `Allow: POST` a odpovědi posílají `Cache-Control: no-store` a `X-Content-Type-Options: nosniff`, aby se v administraci necachoval zastaralý stav. Malé AJAX akce pro obnovu zámku obsahu a řazení obsahu vrací i `request_id`, aby šla chyba v administraci spárovat s technickým logem.
...
- Provozní JSON endpointy `health.php` a `csp-report.php` posílají vedle `Cache-Control: no-store` a `X-Content-Type-Options: nosniff` také `X-Robots-Tag: noindex, nofollow, noarchive` a `Referrer-Policy: no-referrer`, aby monitoring a CSP reporty neputovaly přes cache, index nebo referrer.
