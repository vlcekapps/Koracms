...
- Veřejné i administrační odpovědi posílají `Permissions-Policy`, `Cross-Origin-Opener-Policy: same-origin` a `X-Permitted-Cross-Domain-Policies: none`, takže CMS zakazuje nepoužívaná prohlížečová API, izoluje top-level okna od cross-origin opener vazeb a odmítá staré cross-domain policy soubory. CMS záměrně neblokuje clipboard ani fullscreen, aby zůstala funkční kopírovací tlačítka a legitimní embedy.
