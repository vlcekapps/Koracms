...
- Stavová potvrzení ve veřejné rezervační části účtu, například po zrušení rezervace v `Moje rezervace`, používají `role="status"` s `aria-atomic="true"` a vlastním textovým uzlem přes `aria-labelledby`, aby hláška nebyla pro čtečky obrazovky anonymní.
