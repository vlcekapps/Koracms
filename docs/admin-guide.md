...
- Administrační stažení citlivějších exportů, například SQL záloha databáze, ZIP export galerie nebo ZIP export šablony, posílají `Cache-Control: no-store` a `X-Content-Type-Options: nosniff`, aby se exporty zbytečně necachovaly a prohlížeč je neinterpretoval mimo deklarovaný typ.
