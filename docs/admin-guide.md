...
- `composer ci:basic` navíc obsahuje i izolovaný `test:release-smoke`, který v dočasném snapshot repozitáři skutečně provede `build/release.ps1 -DryRun -SkipCi`, ověří čistý git stav po doběhu a zkontroluje výsledný ZIP i checksum
