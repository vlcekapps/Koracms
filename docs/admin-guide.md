...
- `composer analyse:strict` už na levelu 6 vedle základních helperů pokrývá i `comments`, `content`, `definitions`, `gallery`, `github`, `mail`, `media_library`, `messages`, `presentation`, `revisions`, `stats`, `theme`, `ui`, `webhooks` a `widgets`; ta část kódu proto nově drží přesnější array kontrakty i bez baseline a bez ignore pravidel
