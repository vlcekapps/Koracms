...
Recoverable chyby v administraci se postupně převádějí na strukturovaný `koraLog()` formát. Ukládání článků, přesun článků mezi blogy a ukládání anket tak v technickém logu nespoléhá na surové `error_log()` zprávy, ale přidává `request_id`, metodu, cestu a omezený kontext bez dumpu celé žádosti.
