...
- Přehled rezervací, ruční vytvoření rezervace a detail rezervace v administraci používají sdílené admin CSS třídy a atribut `hidden` pro přepínané části formuláře místo lokálního `<style>` bloku, lokálních `style` atributů nebo JS `element.style` mutací; runtime audit hlídá návrat starého vzoru.
