...
- Nastavení webu v administraci používá nonce CSS třídy pro navigaci sekcí, profil webu, komentáře, notifikace, vlastní kód, náhled loga a favicon i akční tlačítko místo lokálních `style` atributů; runtime audit hlídá návrat starého vzoru.
...
- Přehled rezervací, ruční vytvoření rezervace a detail rezervace v administraci používají nonce CSS třídy a atribut `hidden` pro přepínané části formuláře místo lokálních `style` atributů nebo JS `element.style` mutací; runtime audit hlídá návrat starého vzoru.
...
- Kontrola integrity v administraci používá nonce CSS třídy pro stavové panely, výsledky kontroly, akční řádek a informační blok místo lokálních `style` atributů; runtime audit hlídá, aby se čistě prezentační inline styly do této obrazovky nevracely.
...
- Detail odpovědi formuláře v administraci používá nonce CSS třídy pro workflow pole, GitHub issue návrh, odpověď odesílateli, historii a mazací formulář místo lokálních `style` atributů; runtime audit hlídá, aby se čistě prezentační inline styly do detailu hlášení nevracely.
